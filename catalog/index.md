...
`workspace-atlas` 是一個經治理的專案生態系，目前包含 `_research/` 與 `_development/` 兩類主目錄下的專案。所有專案依類別組織，並以一致的 metadata 追蹤管理。  
`workspace-atlas` is a curated repository ecosystem currently containing projects under the `_research/` and `_development/` top-level directories. All projects are organized by category and tracked with consistent metadata.
...
- **cnogb-abnormal-intervention** — 使用 Transformer 模型進行債券預測與異常偵測。Bond forecasting and anomaly detection using Transformer models.
- **machine-learning-applied-fmri** — 功能性 MRI 資料的機器學習應用研究。Machine learning applications for fMRI analysis.
- **e-learning-fa** — 使用 Python 腳本進行電子學習資料分析。E-learning data analysis using Python tooling.
